...
export async function getPromoExtendeds(state: State, dispatch: React.Dispatch<Action>) {
    ...
    console.debug("getPromoExtendeds: ", promoExtendeds);
    dispatch({ promoExtendeds });
}
...
export async function getTokenAccounts(state: State, dispatch: React.Dispatch<Action>) {
    console.debug("promoExtendeds: ", Object.keys(state.promoExtendeds).forEach(element => {
        console.log(element, (new PublicKey(element)).toString(), state.wallet.publicKey.toString());
    }));
    ...
    console.debug("getTokenAccounts: ", results);
