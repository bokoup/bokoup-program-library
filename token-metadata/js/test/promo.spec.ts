...
describe('promo', () => {
  ...
  it('creates admin settings', async () => {
    ...
    adminSettingsAccount = await tokenMetadata.program.account.adminSettings.fetch(adminSettings) as AdminSettings;
  ...
  it('Creates a promo', async () => {
    ...
    let platformStartAccountInfo = (await tokenMetadata.program.provider.connection.getAccountInfo(adminSettingsAccount.platform));

    const maxMint = 1_000;
    const maxRedeem = 1;
    const expiry = new Date(Date.now() + 1000 * 60 * 60 * 24 * 10);

    promo = await tokenMetadata.createPromo(adminSettingsAccount.platform, metadataData, true, maxMint, maxRedeem, expiry, promoOwner);
    ...
    let platformAccountInfo = await tokenMetadata.program.provider.connection.getAccountInfo(adminSettingsAccount.platform);
  ...
  it('Mints a promo token', async () => {
    const [tokenAccountAccount, mintAccount] = await tokenMetadata.mintPromoToken(promoAccount.mint, promoOwner)
      .then(tokenAccount => Promise.all([
    ...
    expect(Number(tokenAccountAccount.amount)).to.equal(
      1,
      'Token account amount incorrect.',
    );
    expect(Number(mintAccount.supply)).to.equal(
      1,
      'Mint supply incorrect.',
    );
  });
