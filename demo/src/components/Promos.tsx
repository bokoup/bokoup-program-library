...
export const PromoCard: FC<{ mintString: string }> = ({ mintString }) => {
    ...
    // const tokenAccountExists = state.walletConnected && seriesParam.hasOwnProperty("tokenAccount");
    // const tokenAmount = tokenAccountExists ? seriesParam.tokenAccount.amount > 0 : false;


    function getStatsAttributes(): Attribute[] {
        return [
            { "traitType": "outstanding", "value": Number(promoExtended.mintAccount.supply) },
        ...
        <Grid item xs={12} md={6} lg={3}>
            <Card raised>
                <CardHeader
                    ...
                    subheader={
                        <Link sx={{ fontSize: 12, fontWeight: 'medium' }} underline="hover" color="primary.light" href={getExplorerLink(mintString)} target="_blank">
                ...
                <CardMedia
                    component="img"
                    src={promoExtended.metadataJson.image}
                />
                <CardContent>
                    ...
                    <Attributes attributes={getStatsAttributes()} />
                    <Divider sx={{ pt: 1, mb: 1 }} />
                    <Typography sx={{ fontSize: 12 }} component="div">
                        MY STATS
                    </Typography>
...
export const PromoCards: FC = () => {
    ...
    const promoCards = Object.entries(state.promoExtendeds).map(([mintString], i) => <PromoCard key={i} mintString={mintString} />);
...
export const Attributes: FC<{ attributes: Attribute[] }> = ({ attributes }) => {
    return (
        <Box>
            {attributes.map((a, i) => <Chip key={i} sx={{ m: 0.5 }} label={`${a.traitType}: ${a.value}`} />)}
        </Box>
    );
};
